...
var buyItem = document.querySelector('#buyItem');
...
getShoes('/api/shoes', function(allShoes){
  shoeListElem.innerHTML = shoesTemplate({shoeData : allShoes})
  shoes = allShoes;
});

//An API to get the shoe brand
getShoes('/api/shoes/brand', function(data){
    brandListElem.innerHTML = brandTemplate({brandData : data.brands})
})

//An API to get the shoe color
getShoes('/api/shoes/color', function(data){
    colorListElem.innerHTML = colorTemplate({colorData : data.colors})
})

//An API to get the shoe size
getShoes('/api/shoes/size', function(data){
    sizeListElem.innerHTML = sizeTemplate({sizeData : data.sizes})
})

//An API to get the all shoe in the database
...
filterShoe.addEventListener("click", function() {
  var brandClass = document.querySelector("#brandClass");
    ...
    getShoes('/api/shoes/brand/' + brand, function(input) {
      shoeListElem.innerHTML = shoesTemplate({shoeData : input});
    });
});
...
filterShoe.addEventListener("click", function() {
  var colorClass = document.querySelector("#colorClass");
    ...
    getShoes('/api/shoes/color/' + color, function(input) {
      shoeListElem.innerHTML = shoesTemplate({shoeData : input})
      //console.log(input);
    });
})
...
filterShoe.addEventListener("click", function() {
  var sizeClass = document.querySelector("#sizeClass");
    ...
    getShoes('/api/shoes/size/' + size, function(input) {
      shoeListElem.innerHTML = shoesTemplate({shoeData : input})
      //console.log(input);
    });
})

//Event Listener that gets an API for sold shoes
function buyItem(_id) {
  console.log(_id);
    //   buyShoes('/api/shoes/sold/' + _id, function(input) {
    //     shoeListElem.innerHTML = shoesTemplate({shoeData : input})
    // })
